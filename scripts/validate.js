...
// A Class Presentation is the projector surface for Content Delivery and, at
// the same URL, the slides a student reopens to review. The declared list is
// TOPICS in scripts/build-presentations.js, the same editorial-list shape as
// VOLUMES, DECKS and Run of Show's own TOPICS.
//
// Three things are checked, and each one is a failure that renders perfectly.
//
// Reachability, both directions: a declared topic with no generated page is a
// dead card on the lesson, and a unit-N/present-topic-*.html on disk that
// TOPICS does not name is a hand-authored file no rebuild can reproduce. That
// second one is not hypothetical. teacher/present-topic-1-7.html, 2-1 and 2-2
// are exactly that today: hand-made, generated by nothing, checked by nothing,
// and linked from nothing.
//
// The student link: the page is only reachable to a student through that
// topic's classPresentation.url, so a generated deck the data file does not
// point at is a file only findable by typing its name.
//
// The notes prohibition, which is the one that matters most. This page is a
// single URL serving the projector and the student, and that is only safe
// while it carries nothing private. It is static hosting: whatever is in the
// file reaches every browser that opens it, and a CSS-hidden panel ships the
// words anyway. So the page may not carry presenter notes, and may not load a
...
section('Class Presentations');
{
  ...
  if (!Array.isArray(CP_TOPICS)) {
    err(path.join(ROOT, 'scripts', 'build-presentations.js'), 'does not load or export TOPICS');
  } else {
    const declared = new Map();
    for (const topic of CP_TOPICS) {
      declared.set(`${topic.unitDir}/${topic.out}`, topic);

      const dataPath = path.join(ROOT, 'assets', 'data', topic.dataFile);
      totalChecks++;
      const dataSrc = read(dataPath);
      if (!dataSrc) {
        err(dataPath, `Class Presentation declares ${topic.out} from this data file, but it does not exist`);
        continue;
      }
      ...
      totalChecks++;
      if (!new RegExp(`classPresentation[\\s\\S]{0,400}?${topic.out.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`).test(dataSrc)) {
        err(dataPath, `Class Presentation ${topic.out} exists but this data file's classPresentation.url does not point at it, so no student can reach it`);
      }

      const outPath = path.join(ROOT, topic.unitDir, topic.out);
      totalChecks++;
      const pageSrc = read(outPath);
      if (!pageSrc) {
        err(outPath, 'Class Presentation page missing, run: node scripts/build-presentations.js');
        continue;
      }

      // The notes prohibition. Checked on the rendered file rather than the
      // renderer, because the renderer is one edit away from changing and the
      // shipped bytes are what a student downloads.
      for (const [pattern, why] of [
        [/data-notes/, 'carries a data-notes attribute'],
        [/BEHISTORICAL_TEACHING/, 'loads the teacher teaching data'],
        [/teacher\//, 'references the teacher-only directory'],
        [/presenter\s*notes/i, 'mentions presenter notes']
      ]) {
        totalChecks++;
        if (pattern.test(pageSrc)) {
          err(outPath, `Class Presentation ${why}. This page is a student URL: anything in the file reaches every browser that opens it. See scripts/lib/presentation-page.js`);
        }
      }
    }

    for (const unitDir of fs.readdirSync(ROOT).filter(d => /^unit-\d+$/.test(d))) {
      for (const found of glob(path.join(ROOT, unitDir), /^present-topic-.*\.html$/)) {
        totalChecks++;
        if (!declared.has(`${unitDir}/${path.basename(found)}`)) {
          err(found, 'Class Presentation has no entry in build-presentations.js TOPICS, a rebuild cannot reproduce it');
        }
      }
    }
  }
