...
/**
 * The one renderer for Run of Show: a teacher-only "cockpit" view of a
 * lesson's pacing, built entirely from that topic's own `runOfShow` block in
 * its lesson data file (assets/data/lesson-N-N-*.js).
 *
 * Validated by real classroom use on Topic 1.4 (2026-09-05) and now a
 * permanent teacher tool, the same kind of surface as
 * teacher/skills-lens.html: not linked from any lesson page, but covered by
 * its own validate.js reachability check and wired into the offline suite
 * via scripts/test/readings-reproducible.test.js. Coverage of the other 71
 * topics is separate, ongoing content work, the same shape as the eBook's
 * volumes: one topic's runOfShow block authored and added to TOPICS at a
 * time, in scripts/build-run-of-show.js.
 *
 * The class timer persists per topic to localStorage (see saveState/loadState
 * below), so a reload mid-class does not lose pacing progress. It never
 * persists as "running": a reopened page always starts paused, so time spent
 * with the tab closed is never silently counted. Because Green and Silver
 * see the same topic in two different class periods on the same day (see
 * "Green and Silver" in CLAUDE.md), the New Class button clears the saved
 * state entirely, for switching cohorts rather than only recovering a lost
 * tab.
 *
...
<div class="ros-shell">
  ...
  <main class="ros-body">
    ...
    <aside>
      <div class="ros-panel ros-widget" id="ros-timer-widget">
        ...
        <div class="ros-timer-controls">
          <button type="button" id="ros-timer-newclass">New Class (clear all pacing)</button>
        </div>
      </div>
  ...
  <p class="ros-footer-note">Teacher tool, like the Skills Lens &mdash; never linked from a student page. Content above is pulled live from this topic's own lesson data file (runOfShow block), so editing that file is the only way to change what this page says. Pacing progress is saved to this browser only; use New Class to clear it before the next cohort.</p>
</div>
...
(function(){
  ...
  // Persisted across a reload so a teacher who has to refresh mid-class does
  // not lose the Time Log. "running" is deliberately never saved: a reopened
  // page always starts paused, so time the tab spent closed is never counted
  // as elapsed. Green and Silver teach the same topic in two different
  // periods on the same day (see "Green and Silver" in CLAUDE.md), so this is
  // reload recovery, not a same-day carryover; New Class below is what clears
  // it between cohorts.
  var stateKey = 'behistorical-run-of-show-state-' + (DATA.meta.topic || '').replace(/\s+/g, '-');
  function loadState(){
    var saved;
    try { saved = JSON.parse(localStorage.getItem(stateKey)); } catch (e) { return; }
    if (!saved || !Array.isArray(saved.phaseSecondsByIndex) || saved.phaseSecondsByIndex.length !== phases.length) return;
    if (typeof saved.phaseIndex === 'number' && saved.phaseIndex >= 0 && saved.phaseIndex < phases.length) state.phaseIndex = saved.phaseIndex;
    if (typeof saved.slideIndex === 'number' && saved.slideIndex >= 0) state.slideIndex = saved.slideIndex;
    if (typeof saved.elapsed === 'number' && saved.elapsed >= 0) state.elapsed = saved.elapsed;
    if (saved.pacing === 'normal' || saved.pacing === 'condensed' || saved.pacing === 'emergency') state.pacing = saved.pacing;
    state.phaseSecondsByIndex = saved.phaseSecondsByIndex.map(function(n){ return (typeof n === 'number' && n >= 0) ? n : 0; });
  }
  function saveState(){
    try {
      localStorage.setItem(stateKey, JSON.stringify({
        phaseIndex: state.phaseIndex, slideIndex: state.slideIndex, elapsed: state.elapsed,
        pacing: state.pacing, phaseSecondsByIndex: state.phaseSecondsByIndex
      }));
    } catch (e) {}
  }
  function startNewClass(){
    if (!window.confirm('Clear all pacing progress for this topic? Do this between Green and Silver, not mid-class.')) return;
    if (state.running) { clearInterval(timerHandle); state.running = false; document.getElementById('ros-timer-toggle').textContent = 'Start'; }
    state.phaseIndex = 0; state.slideIndex = 0; state.elapsed = 0; state.view = 'phase';
    state.phaseSecondsByIndex = phases.map(function(){ return 0; });
  ...
  document.getElementById('ros-timer-newclass').addEventListener('click', startNewClass);
  ...
  loadState();
  Array.prototype.forEach.call(document.querySelectorAll('[data-pacing]'), function(b){
    b.setAttribute('aria-pressed', b.getAttribute('data-pacing') === state.pacing ? 'true' : 'false');
  });
