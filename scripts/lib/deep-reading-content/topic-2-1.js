...
module.exports = {
  ...
  empires: [
    // ── 01 ────────────────────────────────────────────────────────────────────
    {
      ...
      parts: [
        {
          ...
          blocks: [
            { p: `For most of its history the overland trade crossed a dozen sovereignties, each with its own tolls, its own currency, its own courts and its own bandits, and the traffic rose and fell with how many of those states were currently stable. After the Mongol conquests of the thirteenth century, one family ruled from Korea to the Black Sea. The tolls were fewer, the law was one law, and a merchant robbed in one khanate could seek redress from an authority that also governed the next one.` },
            { p: `The Mongols did not merely tolerate this trade, they were invested in it, in the literal financial sense described in section 03. Steppe rulers had always needed what settled societies produced and had always taxed the exchange; what was new was the scale, and the fact that the men taxing the road at one end and the other were cousins. That is what the term <span class="kt">Pax Mongolica</span> means: not peace in any general sense, since the conquests themselves were catastrophic, but a period in which the commercial arteries of Eurasia ran through one political system.` },
            { note: {
              kind: 'howknow',
      ...
      terms: [
        ['Pax Mongolica', 'The period of roughly a century when one political system governed the length of the overland routes, reducing tolls and making the journey calculable.'],
    ...
    pairs: [
      {
        ...
        body: `Overland trade rose to its peak under the Pax Mongolica, when one political system reduced tolls, offered a single law and made a journey's risk calculable, and it contracted when the Ilkhanate fell after 1335 and the Yuan fell in 1368. Indian Ocean trade in the same period had no dominant power at all: Calicut, Hormuz, Kilwa and Melaka were separately governed, and the system worked anyway. The difference exists because the two routes have different threats. A caravan is exposed to bandits and tolls at every mile of its journey, so it needs the ground policed; a ship is only exposed at its ports, so it needs a harbor that welcomes it and nothing in between.`
      },
      {
        category: 'Economics',
