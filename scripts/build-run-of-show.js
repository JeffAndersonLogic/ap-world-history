...
const MANIFEST_OUT = 'run-of-show-manifest.js';
module.exports = { TOPICS, INDEX_OUT, MANIFEST_OUT };
...
// Public lesson metadata only: lets other teacher command surfaces discover
// which Run of Show pages actually exist without duplicating TOPICS or
// guessing filenames. `code` matches the schedule's topic value (for example
// `1.5`), while `topic` keeps the display label from lesson metadata.
const manifestPath = path.join(ROOT, 'teacher', MANIFEST_OUT);
const manifestData = {
  generatedFrom: 'scripts/build-run-of-show.js TOPICS',
  topics: indexEntries.map(({ topic, title, out }) => ({
    code: String(topic).replace(/^Topic\s+/i, ''), topic, title, out,
  })),
};
const manifestJs = `/* GENERATED by scripts/build-run-of-show.js. Do not hand-edit. */\nwindow.BEHISTORICAL_RUN_OF_SHOW=${JSON.stringify(manifestData)};\n`;
if (CHECK) {
  const existing = fs.existsSync(manifestPath) ? fs.readFileSync(manifestPath, 'utf8') : null;
  if (existing !== manifestJs) {
    console.error(`DRIFT: teacher/${MANIFEST_OUT} does not match TOPICS. Run: node scripts/build-run-of-show.js`);
    drift = true;
  } else {
    console.log(`OK: teacher/${MANIFEST_OUT}`);
  }
} else {
  fs.writeFileSync(manifestPath, manifestJs);
  console.log(`Wrote teacher/${MANIFEST_OUT}`);
}
