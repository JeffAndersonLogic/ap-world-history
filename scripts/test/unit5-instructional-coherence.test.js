...
const contract=require('../lib/unit5-coherence-contract');
...
console.log('\nUnit 5 instructional coherence gate');
for(const [topic,spec] of Object.entries(contract.topics)){
 const k=key(topic),first=`unit-5/first-and-10-topic-${k}-${spec.slug}.html`,deep=`unit-5/deep-reading-topic-${k}-${spec.slug}.html`;
 ...
 let l;try{l=load(topic,spec.slug);}catch(e){failures.push(`${topic}: parse ${e.message}`);console.log(`  ${topic} FAIL parse ${e.message}`);continue;}
...
if(failures.length){console.error(`\nUnit 5 coherence gate failed (${failures.length}).`);failures.forEach(f=>console.error('  - '+f));process.exit(1);}
console.log('\nUnit 5 coherence gate passed.');
