 ...
 *   node scripts/test/modal-focus.unit.js
 *
...
(async () => {
  ...
  //
  // The map module is on every topic; Topic 1.1 happens to carry no Evidence Lab
  // images, so the evidence path is not universal enough to assert here.
  ...
  // Reach it by keyboard rather than clicking, which is the whole point.
  await mapImg.focus();
  ...
  const lectureCard = page.locator('#main-lecture-grid .lecture-topic-card').first();
  ...
  check('Gather still produces a manifest after modal use',
    !!doc && /#BHV\|v=1\|topic=1\.1\|/.test(doc) && /#BHR\|i=01\|slot=map-check-response/.test(doc));
