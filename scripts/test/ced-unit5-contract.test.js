...
const contract=require('../lib/ced-unit5-contract');
const ROOT=path.resolve(__dirname,'..','..'),DATA=path.join(ROOT,'assets','data'),failures=[];
...
console.log('\nUnit 5 CED instructional contract');
for(const [topic,spec] of Object.entries(contract.topics)){
 ...
 for(const scope of ['base','runtime']){
   ...
   (spec.forbiddenKeyConcepts||[]).forEach(k=>{if(codes.includes(k))misses.push(`${scope} forbidden KC ${k}`);});
 }
...
if(failures.length){console.error(`\nUnit 5 CED gate failed (${failures.length}).`);failures.forEach(f=>console.error('  - '+f));process.exit(1);}
console.log('\nUnit 5 CED gate passed.');
