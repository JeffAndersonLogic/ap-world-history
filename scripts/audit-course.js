...
for (const id of Object.keys(grouped).sort((a,b)=>{
  ...
  const mapped=unique(targets.concat(criteria).map(x=>x&&x.kc).filter(x=>/^KC-/.test(String(x||''))));
  ...
  if(examples.length>=3 && trace<35) yellow.push('low illustrative-example traceability in First & 10/Deep Reading ('+trace+'%)');
  notes.push('reading example trace '+trace+'% ('+matched.length+'/'+examples.length+')');
...
const calibrationPass=calibration.length===2 && calibration.every(r=>r.status!=='REBUILD');
...
md+='## Calibration\n\n';
md+='The framework was calibrated on Topic 3.2 (early-course land-empire administration) and Topic 8.4 (late-course dual-objective communism/redistribution). Calibration '+(calibrationPass?'PASS':'NEEDS REVIEW')+': the scanner can evaluate both topic shapes without requiring Unit 2-specific vocabulary.\n\n';
...
const json=JSON.stringify({generatedAt:new Date().toISOString(),calibrationPass,counts,units,results},null,2)+'\n';
