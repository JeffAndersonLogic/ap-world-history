...
This is the same contract the lesson pages settled on, recorded in
...
## NO GOOGLE FORM CAPTURE, AND WHAT IT WOULD TAKE

The desk deliberately has no Submit to Form button.

Read `docs/FORM-CONTRACT.md` before changing this. The short version: the form's
Unit and Topic questions are **dropdowns**, and Google **silently drops** a
prefilled value that is not on the option list. No error, no warning, an empty
cell and a successful submission. Current events is not a CED topic, so there is
no Unit or Topic value to send. Wiring capture here without editing the live
form first would produce submissions that look fine and land half empty, which
is exactly the failure that cost days on 2026-08-03.

If you ever do want it, the form edits come first, by hand, in this order:

1. **Unit dropdown**, add an option. `Current Events` is the obvious string.
2. **Topic dropdown**, add an option, or convert Topic to short answer the way
   Prompt ID already was. Converting is better: it removes the silent-drop
   failure mode permanently instead of adding one more list to maintain.
3. **Response Type**, add `BeCurrent`. Extra options are harmless; missing ones
   are what break.
4. **Prompt ID** needs nothing. It became a short answer question on 2026-08-03
   and accepts any string.
5. Then add the keys to `BH_FORM` in `assets/js/behistorical-form-config.js`,
   and only then build a button.

Verify with a **real submission**, not a URL check. A browser test proves the
URL was built; only a submission proves the field landed.

Worth asking first whether it is wanted at all. The lesson pages went through
exactly this expansion in June and again on 2026-08-03, and reverted both times:
a Submit button is one more thing to forget, and the Sheet is not the grade.
Durability is autosave. Canvas is Copy All My Work. Both already work here.
