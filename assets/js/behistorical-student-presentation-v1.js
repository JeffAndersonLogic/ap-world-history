...
function grid(s){return `<section class="slide grid-slide"><div class="head"><div class="kicker">${esc(s.eyebrow||'')}</div><h2>${esc(s.title||'')}</h2></div><div class="card-grid">${(s.cards||[]).map(c=>`<article><b>${esc(c.title||'')}</b><p>${esc(c.text||'')}</p></article>`).join('')}</div></section>`;}
