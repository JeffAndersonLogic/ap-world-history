...
byId('module-grid').innerHTML=modules.map(m=>{const a=m.link?`window.open('${m.link}','_blank','noopener')`:m.jump?`jumpToSection('${m.jump}')`:`openModule('${m.id}')`;return `<article class="module-card" tabindex="0" role="button" onclick="${a}" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();${a}}" style="--module-img:${bg(m.img)}"><div class="module-label">${m.label}</div><h3>${m.title}</h3><p>${m.desc}</p></article>`;}).join('');
