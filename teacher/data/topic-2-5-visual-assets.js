(function(){
'use strict';
const css=document.createElement('style');
css.id='topic25-visual-assets';
css.textContent=`
.hero-slide .media{background:#030404}
.hero-slide img{object-fit:contain!important;object-position:center center!important;background:#030404}
.hero-slide .copy{padding:1.2rem 1.35rem;background:rgba(4,6,7,.42);border-left:3px solid var(--gold);backdrop-filter:blur(3px);text-shadow:0 2px 18px rgba(0,0,0,.78)}
.hero-slide.right .copy{left:auto!important;right:4.5%!important;width:min(43%,760px)!important}
.hero-slide.right .veil{background:linear-gradient(270deg,rgba(3,5,6,.94) 0%,rgba(3,5,6,.72) 34%,rgba(3,5,6,.16) 64%,rgba(3,5,6,.04) 82%)!important}
.image-canvas{overflow:hidden!important}
.image-canvas img{width:auto!important;height:auto!important;max-width:100%!important;max-height:100%!important;object-fit:contain!important;object-position:center center!important;display:block!important}
.hero-slide:has(img[src*="Caravane_Marco_Polo"]) .copy{left:auto!important;right:4%!important;bottom:8%!important;width:min(40%,720px)!important}
.hero-slide:has(img[src*="TabulaRogeriana"]) .copy{left:auto!important;right:4%!important;bottom:8%!important;width:min(46%,820px)!important}
.project-mode .hero-slide .copy{padding:1.5rem 1.7rem}
.project-mode .hero-slide h2{font-size:clamp(3.6rem,5vw,6rem)}
.project-mode .hero-slide.right .copy{width:min(44%,880px)!important}
`;
document.head.appendChild(css);
})();
