 ...
 *
 * Art direction: newsprint ground, a fine engraving hatch and survey grid, then
 * one motif per event in gunmetal ink. Orange marks NOW, so within each
 * composition it is reserved for the present-day end: the newest nodes, the
 * hottest bars, the live boundary. These render at 96px tall in a card header.
 */
...
const STEEL   = '#4A555D';
// Orange marks NOW, so in every motif it is reserved for the present-day end of
// the composition: the newest nodes, the hottest bars, the live boundary.
const SIGNAL  = '#FF6A13';
const SIGNAL_2= '#D9530C';
// Printed on newsprint, not on a dark tile. The ground is paper and the motif
// is the ink, so these read as engraved diagrams in a broadsheet rather than as
// photographs that never arrived.
const GROUND_LT = '#EFEBE1';
...
function wrap(w, h, id, motif) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" role="img" aria-hidden="true">${ground(w, h, id)}${motif}</svg>`;
}
...
function socialMedia(w, h) {
  ...
  nodes.forEach(([x, y], i) => {
    const r = i % 3 === 0 ? 13 : 8;
    s += `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}" fill="${i < 6 ? SIGNAL_2 : SIGNAL}" fill-opacity="${i < 6 ? 0.95 : 0.7}"/>`;
  });
  return s + '</g>';
}
...
function whiteCollar(w, h) {
  ...
  for (let r = 0; r < 4; r++) {
    for (let c = 0; c < 9; c++) {
      ...
      s += `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${sz.toFixed(1)}" height="${(sz * 0.7).toFixed(1)}" fill="${c < 4 ? STEEL : SIGNAL}" fill-opacity="${(0.85 - decay * 0.55).toFixed(2)}"/>`;
    }
  }
  for (let i = 0; i < 26; i++) {                                 // the particles
    const x = w * (0.58 + rand() * 0.40), y = h * (0.12 + rand() * 0.78);
    s += `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(2 + rand() * 4).toFixed(1)}" fill="${SIGNAL_2}" fill-opacity="0.5"/>`;
  }
  return s + '</g>';
}
...
function territory(w, h) {
  ...
  s += `<path d="M0 ${h * 0.46} L${w} ${h * 0.34} L${w} ${h * 0.90} L0 ${h}" fill="${SIGNAL}" fill-opacity="0.46"/>`;
  s += `<path d="M${w * 0.52} 0 L${w * 0.60} ${h}" stroke="${SIGNAL_2}" stroke-width="4" stroke-dasharray="18 14" fill="none" stroke-opacity="0.9"/>`;
  for (let i = 1; i <= 5; i++) {
...
function iran(w, h) {
  let s = '<g fill="none">';
  for (let i = 0; i < 6; i++) {
    s += `<circle cx="${w * 0.30}" cy="${h * 0.56}" r="${w * (0.08 + i * 0.075)}" stroke="${i < 3 ? SIGNAL : STEEL}" stroke-opacity="${(0.55 - i * 0.06).toFixed(2)}" stroke-width="2.5"/>`;
  }
  s += `<path d="M0 ${h * 0.80} L${w * 0.34} ${h * 0.80} L${w * 0.46} ${h * 0.58} L${w * 0.78} ${h * 0.58} L${w * 0.88} ${h * 0.36} L${w} ${h * 0.36}" stroke="${SIGNAL_2}" stroke-width="7" stroke-opacity="0.85"/>`;
  s += `<circle cx="${w * 0.30}" cy="${h * 0.56}" r="11" fill="${SIGNAL_2}"/>`;
  return s + '</g>';
}
...
function climate(w, h) {
  ...
  for (let i = 0; i < 22; i++) {
    ...
    s += `<rect x="${x.toFixed(1)}" y="${(h * 0.88 - bh).toFixed(1)}" width="${(w * 0.028).toFixed(1)}" height="${bh.toFixed(1)}" fill="${t > 0.62 ? SIGNAL_2 : STEEL}" fill-opacity="${(0.4 + t * 0.5).toFixed(2)}"/>`;
  }
  s += `<path d="M${w * 0.05} ${h * 0.76} Q ${w * 0.55} ${h * 0.72} ${w * 0.78} ${h * 0.42} T ${w * 0.97} ${h * 0.16}" stroke="${SIGNAL}" stroke-width="5" fill="none" stroke-opacity="0.95"/>`;
  return s + '</g>';
}

// 06 Immigration: directed movement crossing a hard boundary.
function immigration(w, h) {
  let s = `<g><path d="M${w * 0.62} 0 L${w * 0.62} ${h}" stroke="${SIGNAL_2}" stroke-width="5" stroke-opacity="0.9"/>`;
  for (let i = 0; i < 7; i++) {
    const y = h * (0.14 + i * 0.12);
    const x2 = w * (0.30 + (i % 3) * 0.13);
    s += `<path d="M${w * 0.05} ${y} L${x2} ${y}" stroke="${i % 2 ? SIGNAL : STEEL}" stroke-width="3" stroke-opacity="0.7"/>`;
    s += `<path d="M${x2} ${y} l-16 -9 l0 18 Z" fill="${i % 2 ? SIGNAL : STEEL}" fill-opacity="0.9"/>`;
  }
  for (let i = 0; i < 4; i++) {
    const y = h * (0.22 + i * 0.19);
    s += `<path d="M${w * 0.66} ${y} L${w * 0.94} ${y}" stroke="${SIGNAL_2}" stroke-width="3" stroke-opacity="0.45"/>`;
  }
  return s + '</g>';
}
...
function heroSpine(w, h) {
  ...
  for (let i = 0; i < stops; i++) {
    ...
    const t = i / (stops - 1);
    const r = 20 - t * 9;
    s += `<circle cx="${x}" cy="${y.toFixed(1)}" r="${r.toFixed(1)}" fill="${t < 0.34 ? SIGNAL_2 : SIGNAL}" fill-opacity="${(0.95 - t * 0.45).toFixed(2)}"/>`;
