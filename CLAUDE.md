...
- `npm test`, the gate. Runs `validate.js` plus the dependency-free tests in
...
- `node scripts/build-exam.js`, rebuild the Canvas QTI package for every exam in
  `scripts/lib/exam-content/`. `--check` fails on drift without writing, which is
  what the offline suite runs. `--bundle` also writes the importable `.zip`, which
  is gitignored because it is derived and two megabytes of it is image bytes.
  Never hand-edit `exams/**/*.xml`. See "The Examinations" below.
- `node scripts/check-exam-cues.js [--stats]`, the gate on exam item engineering:
  answer-length cueing, absolutist language concentrated in distractors, key
  letter balance, cross-references between items, stimulus images that are real
  repository files, and coverage against the learning targets. In the offline
  suite. It fails a version 2 shaped exam on every one of those counts. What it
  cannot check is whether a key is historically right, which stays a teacher's
  judgment, the same split as `check-style.js`.
...
An exam is generated, like everything else. Content lives in
`scripts/lib/exam-content/<exam>.js`, `scripts/build-exam.js` renders the Canvas
QTI package into `exams/<id>/`, and the XML is never edited by hand.

**Version 3 of the Foundations exam exists because of a measured failure, and
the failure is worth keeping written down.** Version 2 was generated by a
language model, read well, and was rated highly by a five-persona review. All
forty of its answer keys were historically correct. It was still unusable:

- **The key was strictly the longest option in 38 of 40 items.** A student who
  knew nothing and always picked the longest option scored 95%. On the eight
  skills items the key ran two to three times the length of its distractors.
- **Absolutist words** appeared in 22% of distractors and 2% of keys, so "cross
  off whatever sounds too strong" was a second free signal pointing the same way.
- **Three items told students to read a Southern Song map.** The image was
  `instructional-maps/foundations-5.svg`, which is the six AP regions map: no
  Yangtze, no Lin'an, no Quanzhou. The alt text describing a Song map had been
  written to match a plausible filename.
- **Eight items asked which statement "best demonstrates contextualization"**, or
  causation, or comparison. That tests whether a student recognizes a vocabulary
  word rather than whether they can do the thing.
- **Christianity never appeared at all.** Islam, Hinduism and Daoism appeared
  only as wrong answers, and the Mediterranean sea lanes were never a correct
  answer. Every one of those sits inside a stated Foundations learning target.
- **Twelve items hotlinked images**, two of them to commons.wikimedia.org, which
  returned HTTP 429 under test.

**None of that is visible by reading, and all of it is trivial to measure.** That
is the shape of failure this repo builds gates for, so `check-exam-cues.js` runs
in the offline suite and fails on every one of the mechanical counts.

### What the gate enforces, and what it leaves alone

Enforced, because each one is invisible to a reader and each one lets a student
score without knowing anything: the length cue, absolutist language concentrated
in distractors, key letter balance, an item referring to another by number,
a stimulus image that is not a real repository file, and the coverage floors and
required terms the exam declares for itself.

Not enforced, because they are teaching decisions: how many items, how they are
weighted, how hard they are, and whether a key is historically defensible.

**The length rule is measured with a tie band, and the band is not a loophole.**
Options within a few characters look the same on the page, and a student picking
"the longest" is really guessing among them, so the naive-strategy score counts
those as a guess rather than a hit. Without the band an item whose key beats the
next option by one character scores as fully cued, which is untrue and would push
an author into padding options to no purpose. V2's keys beat the next option by
30 to 100 characters, so the band changes nothing about that verdict: V2 still
scores 89% and still fails. **Re-run the V2 fixture check before loosening any
threshold in that file.** A gate tuned until the current draft passes is not a
gate.

**Option order is arranged by machine, not by the author.**
`scripts/lib/exam-arrange.js` is the one implementation and both the builder and
the auditor call it, for the same reason `canvas-parse-core.js` is inlined into
the Skills Lens rather than copied. Writing an item means writing the correct
answer first, so the first draft of V3 put the key at A in 37 of 40 items without
anybody intending it. The arrangement is a pure function of the item ids, so it
is stable across builds and machines, and letters come out exactly even. Canvas
answer shuffling is a good setting on top of this and does not replace it: it can
be switched off, it does nothing for a printed copy, and it does nothing for the
answer key a teacher reads out of the repo.

**Every item carries its whole source block.** Canvas delivers one question at a
time and a teacher may turn shuffling on, so an item that leaned on a source
shown by an earlier item would put a question on screen about a picture that is
not there. It is the same reason no item may name another by number.

**Stimulus images are repository files, referenced by absolute URL on the course
site.** That is not the hotlinking that made V2 fragile: these sit on the host
that already serves every lesson, reading and map the class opens daily, so if it
is unreachable the exam is the least of that day's problems. `--images=local`
rewrites them to relative paths and copies the files into the package, because
Canvas resolves bundled QTI images inconsistently across instances and that
choice belongs to whoever runs the import.

**Check what a map actually shows before writing a question about it.** The
`caption` field in the Foundations data files is the reference, and it is what
would have caught the Southern Song error in about a minute.

**The passages are written for the examination and say so on the page.** Nothing
is attributed to a historian who did not write it, and no quotation, statistic or
date is invented, which is the same rule BeCurrent states about reporting. Where
a real excerpt would teach better, swap one in and keep the label honest.

**Per-answer feedback is the part students actually read.** V2 shipped none, so a
student who missed an item learned only that they had missed it. Every distractor
in V3 carries a sentence saying what is wrong with it.
