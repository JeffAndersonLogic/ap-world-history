...
- `node scripts/build-foundations-exam.js`, render the 40-question Foundations unit exam from `scripts/lib/foundations-exam-v1.js` into `foundations/assessments/`: the Canvas QTI zip, a CSV for spreadsheets and CSV-to-QTI converters, the printable student exam, and the teacher key with rationales and a blueprint. `--check` fails on drift without writing, which is what the offline suite runs. Never hand-edit the four outputs; edit the content module and rebuild. Canvas does not import quiz questions from a CSV, so the zip is the import path: Settings, Import Course Content, QTI .zip file.
- `node scripts/test/foundations-exam.test.js`, prove the four exam artifacts still match the content module and that the exam does not leak its own answers. In the offline suite. It asserts the structural contract (forty contiguous items, four distinct choices, exactly one key, every QTI `varequal` naming a choice that exists) and two statistical ones: the key spread evenly across all four positions, and the correct answer almost never the longest choice. Both statistical checks exist because the first draft failed them, with 23 of 40 keys on B, none on D, and 34 of 40 correct answers the longest option. Nothing about reading the exam straight through reveals either.
