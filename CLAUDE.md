...
- `node scripts/test/canvas-topic-recovery.test.js`, offline check that a submission whose record footer is missing still lands on a topic. Students delete that footer, so the parser has a recovery path, and on Foundations the recovery silently failed: the renderer writes `AP World History, FOUNDATIONS 1` and the pattern only accepted `Topic 1.1` or `F1`. Responses arrived with an empty `topic_id` and dropped out of every topic-filtered panel, which looked exactly like a class that had not done the work. It was 16 of 115 students on f1 and all but 6 of 130 on f0. Both renderers now also print a `Topic code:` line in the heading, above the first module label, because the heading is the part students keep.
